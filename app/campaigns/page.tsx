...
export default function Campaigns() {
  ...
  async function handleSave() {
    ...
    const res = await fetch('/api/campaigns', {
      ...
      body: JSON.stringify({
        name: form.name, advertiser: form.advertiser, agency: form.agency,
        flight_start: form.flight_start, flight_end: form.flight_end,
        booked_spots: parseInt(form.booked_spots), booked_stations: form.stations,
        status: 'active',
      })
    })
    setSaving(false)
  ...
  const inputStyle = { width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid #E2E8F0', fontSize: 14, outline: 'none', boxSizing: 'border-box' as const, fontFamily: 'system-ui' }
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: '2rem', width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. FCMB April 2026" style={inputStyle} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <label style={labelStyle}>Advertiser <span style={{ color: '#EF4444' }}>*</span></label>
                  <input value={form.advertiser} onChange={e => set('advertiser', e.target.value)} placeholder="e.g. FCMB" style={inputStyle} />
                </div>
                <div>
              ...
              <div>
                ...
                <input type="number" value={form.booked_spots} onChange={e => set('booked_spots', e.target.value)} placeholder="e.g. 60" style={inputStyle} />
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: '12px', borderRadius: 8, border: '1px solid #E2E8F0', background: 'white', fontSize: 14, cursor: 'pointer', color: '#64748B' }}>Cancel</button>
              <button onClick={handleSave} disabled={saving} style={{ flex: 2, padding: '12px', borderRadius: 8, border: 'none', background: saving ? '#94A3B8' : '#028090', color: 'white', fontSize: 14, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
      ...
      <div style={{ padding: '2rem', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 14, color: '#64748B' }}>Track booked vs. aired spots — live from play event database</div>
          </div>
          <button onClick={() => setShowModal(true)} style={{ background: '#028090', color: 'white', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>+ New campaign</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
          {[
            ...
            { label: 'Total spots booked', value: campaigns.reduce((a,c) => a + (c.booked_spots || 0), 0) },
            { label: 'Total spots aired',  value: campaigns.reduce((a,c) => a + (c.aired_spots || 0), 0) },
          ].map(card => (
        ...
        {loading ? (
          <div style={{ background: 'white', borderRadius: 12, padding: '3rem', border: '0.5px solid #E2E8F0', textAlign: 'center', color: '#64748B' }}>Loading campaigns...</div>
        ) : campaigns.length === 0 ? (
          <div style={{ background: 'white', borderRadius: 12, padding: '3rem', border: '0.5px solid #E2E8F0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, color: '#64748B', marginBottom: 24 }}>Create your first campaign to start tracking booked vs aired spots.</div>
            <button onClick={() => setShowModal(true)} style={{ background: '#028090', color: 'white', border: 'none', borderRadius: 8, padding: '12px 32px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>+ Create first campaign</button>
          </div>
        ) : (
                ...
                <div key={i} style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '0.5px solid #E2E8F0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 13, color: '#64748B', marginBottom: 4 }}>{c.advertiser} {c.agency ? '· ' + c.agency : ''}</div>
                      <div style={{ fontSize: 12, color: '#94A3B8' }}>{c.flight_start} to {c.flight_end} · {(c.booked_stations || []).join(', ')}</div>
                    </div>
                    <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: 11, color: '#64748B', marginBottom: 4 }}>Booked</div>
                        <div style={{ fontSize: 24, fontWeight: 700, color: '#0D1B3E' }}>{c.booked_spots}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: 11, color: '#64748B', marginBottom: 4 }}>Aired</div>
                        <div style={{ fontSize: 24, fontWeight: 700, color: '#0D1B3E' }}>{c.aired_spots ?? 0}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                      ...
                      {c.shortfall > 0 && (
                        <div style={{ background: '#FEF2F2', borderRadius: 8, padding: '8px 14px', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 18, fontWeight: 700, color: '#EF4444' }}>-{c.shortfall}</div>
                        </div>
                      )}
                    </div>
                  </div>
