...
export default async function Dashboard() {
  ...
  const { count: missedSpots } = await supabase
    .from('discrepancies')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'open')
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          ...
          <div style={{ background: 'white', borderRadius: 12, padding: '1.5rem', border: '0.5px solid #E2E8F0' }}>
            ...
            {recentPlays?.map((play, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '10px 0', borderBottom: '0.5px solid #F1F5F9' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 500, fontSize: 13, color: '#0D1B3E' }}>{play.ad_title ?? 'Unknown ad'}</div>
                  <div style={{ fontSize: 12, color: '#64748B' }}>{play.station_name} · {play.advertiser ?? '—'}</div>
                </div>
                <div style={{ textAlign: 'right', minWidth: 80 }}>
                  <div style={{ fontSize: 12, color: '#64748B' }}>
                    {new Date(play.play_start).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                  </div>
